...
func AmbilRedis() (*JsonResponse, error) {
	redisClient := redis.NewClient(&redis.Options{
		Addr:     "localhost:6379",
		Password: "",
		DB:       0,
	})
	cachedBuku, err := redisClient.Get("buku").Bytes()
	...
	if err != nil {
		...
		cachedBuku, err = json.Marshal(dbBuku)

		if err != nil {
			return nil, err
		}

		err = redisClient.Set("buku", cachedBuku, 10*time.Second).Err()

		if err != nil {
			return nil, err
		}

		response = JsonResponse{Data: dbBuku, Source: "PostgreSQL"}

		return &response, err
	}
	buku := []Buku{}

	err = json.Unmarshal(cachedBuku, &buku)

	if err != nil {
		return nil, err
	}

	response = JsonResponse{Data: buku, Source: "Redis Cache"}

	return &response, nil
}

func dataDb() ([]Buku, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&p.ID, &p.Judul_buku, &p.Penulis, &p.Tgl_publikasi)
